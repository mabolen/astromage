...
export class Animator {
    ...
    async animateResourceCard(player: Player, playerRef: any, activeCard: number | null) {
        ...
        if (activeCard) {
            resNames.forEach(s => {
                ...
                if (player.stats[s] < playerRef.current[s] && (playerRef.current[s] - player.stats[s]) !== player.hand[activeCard].cost) statsObj.resourcesDown.push(s)
            })
        }
